...
PanelWindow {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: function(mouse) {
                                        if (musicPanel.length > 0 && !seekProc.running) {
                                            ...
                                            seekProc.command = ["playerctl", "--player=%any", "position", seekPos.toString()]
    ...
    Process {
        id: musicStatusProc
        command: ["playerctl", "--player=%any", "status"]
        stdout: SplitParser {
            onRead: data => {
    ...
    Process {
        id: musicTitleProc
        command: ["playerctl", "--player=%any", "metadata", "title"]
    ...
    Process {
        id: musicArtistProc
        command: ["playerctl", "--player=%any", "metadata", "artist"]
    ...
    Process {
        id: musicPosProc
        command: ["playerctl", "--player=%any", "position"]
        stdout: SplitParser {
            onRead: data => {
    ...
    Process {
        id: musicLenProc
        command: ["sh", "-c", "playerctl --player=%any metadata mpris:length 2>/dev/null | awk '{print $1/1000000}'"]
        stdout: SplitParser { onRead: data => musicPanel.length = parseFloat(data.trim()) || 0 }
    }

    Process {
        id: playPauseProc
        command: ["playerctl", "--player=%any", "play-pause"]
        onExited: { if (!musicStatusProc.running) musicStatusProc.running = true }
    }

    Process {
        id: nextProc
        command: ["playerctl", "--player=%any", "next"]
        onExited: { if (!musicStatusProc.running) musicStatusProc.running = true }
    }

    Process {
        id: prevProc
        command: ["playerctl", "--player=%any", "previous"]
        onExited: { if (!musicStatusProc.running) musicStatusProc.running = true }
    }

    Process {
        id: seekProc
        command: ["playerctl", "--player=%any", "position", "0"]
    }
}
